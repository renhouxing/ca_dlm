...
class AttnMaskLM(LM):
    ...
    def generate_until(self, requests):
        ...
        for i, req in enumerate(requests, start=1):
            ...
            output_ids, steps = self.mask_diffusion(
                input_ids=input_ids,
        ...
        print(f"Average steps per generation: {total_steps / len(requests):.2f}")
    ...
    def select_tokens(self, weights, corr, entropy):
        ...
        while len(selected) < weights.shape[0]:
            coefs = weights.clone()
            ...
            new_sum = ent_sum + entropy[k].item()
            new_max = max(ent_max, entropy[k].item())

            if len(selected) == 0 or (new_sum - new_max <= self.gamma):
                selected.append(k)
                ent_sum = new_sum
                ent_max = new_max
            else:
    ...
    def mask_diffusion(
        ...
        while answer_mask.any():
            ...
            entropy = entropy[answer_mask]

            selected = self.select_tokens(weights, corr, entropy)
            ...
            string = self.tokenizer.decode(input_ids[input_len:])

            full = False
            for s in ['```', '<|eot_id|>', '<|im_end|>']:
                if s in string:
                    idx = string.index(s)
                    if self.tokenizer.mask_token not in string[:idx]:
                        full = True
                        break
            if full:
                break
